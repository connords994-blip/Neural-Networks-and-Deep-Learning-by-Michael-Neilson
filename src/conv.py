...
def ensemble(nets): 
    ...
    (and faster) to batch the vote in a single pass.  But this works.
    ...
    for net in nets:
        net.test_predictions = list(np.concatenate([
            net.predict(
                test_x[i*net.mini_batch_size: (i+1)*net.mini_batch_size]).numpy()
            for i in range(1000)]))
...
def plot_filters(net, layer, x, y):
    ...
    filters = net.layers[layer].w.detach().numpy()
    fig = plt.figure()
